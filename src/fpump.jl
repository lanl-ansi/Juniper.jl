...
function generate_mip(optimizer, m, nlp_sol, tabu_list, start_fpump)
    ...
    if num_sols > 0
        z1 = [
            add_constrained_var(mip_optimizer, MOI.ZeroOne()) for
            _ in 1:m.num_disc_var, _ in 1:num_sols
        ]
        z2 = [
            add_constrained_var(mip_optimizer, MOI.ZeroOne()) for
            _ in 1:m.num_disc_var, _ in 1:num_sols
        ...
        v = tabu_list.sols
        for k in 1:num_sols, j in 1:m.num_disc_var
            i = m.disc2var_idx[j]
            ...
            MOI.add_constraint(
                mip_optimizer,
                MA.@rewrite((lbi - v[k][j]) * z1[j, k] + z2[j, k] - x[i]),
                MOI.LessThan(-v[k][j]),
            )
            MOI.add_constraint(
                mip_optimizer,
                MA.@rewrite((ubi - v[k][j]) * z2[j, k] - z1[j, k] - x[i]),
                MOI.GreaterThan(-v[k][j]),
            )
        end
        for k in 1:num_sols
            MOI.add_constraint(
...
function generate_nlp(optimizer, m, mip_sol, start_fpump; random_start = false)
    ...
    obj = sum(
        (x[m.disc2var_idx[i]] - mip_sol[m.disc2var_idx[i]])^2 for
        i in 1:m.num_disc_var
    )
...
function generate_real_nlp(optimizer, m, sol; random_start = false)
    ...
    index_map = MOI.copy_to(
        nlp_optimizer,
        FixVariables(IntegerRelaxation(optimizer), disc_vals),
    )
    x = [
        index_map[vi] for vi in MOI.get(optimizer, MOI.ListOfVariableIndices())
    ]
...
function add!(t::TabuList, m, sol)
    t.sols[t.pointer] = [sol[m.disc2var_idx[i]] for i in 1:m.num_disc_var]
    t.pointer += 1
...
function fpump(optimizer, m)
    ...
    while !are_type_correct(nlp_sol, m.var_type, m.disc2var_idx, catol) &&
        ...
        if any(
            ...
            for vi in 1:m.num_disc_var
                vidx = m.disc2var_idx[vi]
                mip_sol[vidx] = round(mip_sol[vidx])
            end
        end
        if mip_status != MOI.OPTIMAL
            @warn "MIP couldn't be solved to optimality. Terminated with status: " *
                  string(mip_status)
            break
        end
        ...
        nlp_status, nlp_sol, nlp_obj =
            generate_nlp(optimizer, m, mip_sol, start_fpump)
        if !state_is_optimal(
            nlp_status;
        ...
        if are_type_correct(
            ...
            m.disc2var_idx,
            catol * 1000,
        ) || isapprox(nlp_obj, 0.0; atol = catol)
            real_status, real_sol, real_obj =
            ...
            if state_is_optimal(real_status) || (
                only_almost_solved(real_status) &&
        ...
        if !isapprox(nlp_obj, 0.0; atol = catol) &&
           isapprox(nlp_obj, 0.0; atol = 10 * catol)
            atol_counter += 1
        else
            atol_counter = 0
        end

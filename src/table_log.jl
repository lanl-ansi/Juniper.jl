function get_table_config(opts)
    ...
    if (opbs == :StrongPseudoCost || opbs == :Reliability) &&
       opts.strong_restart
...
function print_table(
    ...
    table_line, table_arr = get_table_line(
        p,
        tree,
        node,
        step_obj,
...
function get_table_line(
    ...
    if (
        opbs != :StrongPseudoCost ||
        counter > tree.options.strong_branching_nsteps
    ...
    if isdefined(tree, :incumbent) && found_both == 2
        if tree.incumbent.objval != tree.best_bound
            while round(tree.incumbent.objval; digits = precision) ==
                  round(tree.best_bound; digits = precision) && precision < 10
                if length(string(tree.incumbent.objval, precision + 1)) >
                   min_field_chars - 2
                    break
                end
                if length(string(tree.best_bound, precision + 1)) >
                   min_field_chars - 2
                    break
                end
                precision += 1
                ...
                if isnan(val)
                    ...
                    if length(string(val)) > field_chars[i]
                        if val > 0 && val < tree.options.mip_gap * 100
                            val =
                                "< " * string(tree.options.mip_gap * 100) * "%"
                        elseif val > 1000
                            val = ">>"
                        else
                            val = string(val) * "%"

...
function branch_strong_on!(m,opts,step_obj,
    ...
    while restart 
                ...
                if p != myid() || np == 1
                    @async begin
                        while true
                            ...
                            l_nd,r_nd = branch!(m, opts, strong_step_obj, counter, disc2var_idx; temp=true)
                            # println("var_idx: ", var_idx)
                            # println("node.var_idx: ", node.var_idx)
                            ...
                            if !state_is_optimal(l_nd.relaxation_state) || !state_is_optimal(r_nd.relaxation_state)
                                ...
                                local_need_to_resolve, local_restart, new_infeasible_disc_vars, local_set_to_last_var = init_strong_restart!(node, var_idx, disc_var_idx, l_nd, r_nd, reasonable_disc_vars, infeasible_disc_vars, strong_obj.left_node, strong_obj.right_node, strong_restart, strong_obj.max_gain_var, opts.atol, m.var_type, disc2var_idx)
        ...
        # TODO: Combine the gain information
